...
export const bottomSheetOverlay = style({
  ...
  '@media': {
    '(max-width: 639px)': {
      ...
      bottom: 'max(var(--tr-bottom-navbar-offset, calc(96px + env(safe-area-inset-bottom, 0px))), calc(96px + env(safe-area-inset-bottom, 0px)))',
    },
  },
});
  ...
  '@media': {
    '(max-width: 639px)': {
      /* Sit between dynamic top & bottom nav offsets (runtime measured),
        with a strict minimum of 96px + safe area to prevent hiding behind nav. */
      top: 'max(var(--tr-top-navbar-offset, 57px), 57px)',
      bottom: 'max(var(--tr-bottom-navbar-offset, calc(96px + env(safe-area-inset-bottom, 0px))), calc(96px + env(safe-area-inset-bottom, 0px)))',
      maxHeight: 'none',
      overflow: 'hidden',
