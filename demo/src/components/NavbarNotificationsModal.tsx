...
export function NavbarNotificationsModal({ notificationsList, activityItems = [], onClose, onNavigate }: NotificationsModalProps) {
  ...
  const hasUnread = notificationsList.some((n) => !(n.read ?? n.is_read));

  // Mark all notifications as read when the modal opens
  useEffect(() => {
    if (!hasUnread) return;
    api.post('/user-notifications/mark-all-read/').then(() => {
  ...
  const header = (
    <div className={s.modalHeaderRow}>
      <div className="flex-1">
        <div className={s.modalTitle}>Notificaties</div>
      </div>
