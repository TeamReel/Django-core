...
export function useWorkflowTemplates(): UseWorkflowTemplatesReturn {
  ...
  useEffect(() => {
    async function fetchTemplates() {
      try {
        setLoading(true);
        const data = await api.get<Record<string, unknown>>('/workflows/templates/?is_active=true');
  ...
  useEffect(() => {
    async function fetchInstances() {
      try {
        ...
        const data = await api.get<Record<string, unknown>>(`/workflows/instances/?${params.toString()}`);
...
export function useTransitionHistory(instanceId: number | string | null): UseTransitionHistoryReturn {
  ...
  useEffect(() => {
    ...
    async function fetchHistory() {
      try {
        setLoading(true);
        const data = await api.get<Record<string, unknown>>(`/workflows/history/?instance=${instanceId}&ordering=-created_at`);
