...
**Independent Test**: Run migrations â†’ seed test data â†’ verify constraints (last admin protection, duplicate prevention) â†’ query performance (<50ms)

**Included Subtasks**:

- [x] **T001**: Extend `Project` model with `is_private` boolean field (default=False)
...
- [x] **T002**: Create `ProjectMembership` model with soft delete [P]
...
- [x] **T003**: Create `ProjectInvite` model with token-based acceptance
...
- [x] **T004**: Create `ProjectMembershipPromotion` model with approval workflow
...
- [x] **T005**: Add model validation rules [P]
...
- [x] **T006**: Register models in Django Admin [P]
...
**Success Criteria**:
- [x] All 4 models created and migrated successfully
- [x] Indexes created (verify with `\d+ project_membership` in psql)
- [x] Validation prevents last admin removal
- [x] Django Admin shows all models with correct filters
- [x] Test data seeding works without errors
...
**Independent Test**: Call `get_project_role(user, project)` â†’ verify explicit membership overrides â†’ verify private project enforcement â†’ verify cache hit rate >80% â†’ verify resolution time <50ms p95

**Included Subtasks**:

- [x] **T007**: Create `PermissionResolutionService` class
  - Method: `get_project_role(user_id, project_id) -> PermissionResult`
  - Implements 5-step resolution: explicit membership â†’ private check â†’ org membership â†’ emergency override â†’ no_access
...
- [x] **T008**: Implement hybrid caching strategy
...
- [x] **T009**: Create cache invalidation signals
  - Signal handlers in `apps/projects/signals.py`:
    - `@receiver(post_save, sender=ProjectMembership)` â†’ invalidate user's permissions
    - `@receiver(post_delete, sender=ProjectMembership)` â†’ invalidate user's permissions
    - `@receiver(post_save, sender=Project)` â†’ if `is_private` changed, invalidate all project permissions
  - Bulk invalidation: `invalidate_project_permissions(project_id)` clearing all users

- [x] **T010**: Add B08 integration for permission checks
  ...
  - Maps roles to permissions: viewer â†’ `[projects.view]`, editor â†’ `[projects.view, projects.edit]`, admin â†’ `[projects.view, projects.edit, projects.delete, projects.manage_members]`
  - Location: `apps/access_control/managers.py` (extend existing)

- [x] **T011**: Add performance instrumentation
...
**Success Criteria**:
- [x] Permission resolution returns correct role for all scenarios (explicit, implicit, private, no_access)
- [x] Cache hit rate >80% after warmup (100 checks)
- [x] Resolution time <50ms at p95 (measure with `pytest-benchmark`)
- [x] Cache invalidates correctly (add membership â†’ check cache cleared)
- [x] Prometheus metrics visible at `/metrics` endpoint
...
**Independent Test**: Login as project admin â†’ search org member by name â†’ assign editor role â†’ verify immediate access â†’ verify notification sent

**Included Subtasks**:

- [x] **T012**: Create `ProjectMembershipSerializer` [P]
...
- [x] **T013**: Create `ProjectMembershipViewSet` [P]
...
- [x] **T014**: Create `MembershipService` for business logic
...
- [x] **T015**: Add B09 audit events integration [P]
...
- [x] **T016**: Add B16 notification triggers [P]
...
- [x] **T017**: Frontend: MemberList component
...
- [x] **T018**: Frontend: AddMemberModal component
  ...
  - "Add Member" button â†’ POST to `/projects/{id}/members`
...
**Success Criteria**:
- [x] Can add org member via API: `POST /projects/{id}/members {"user_id": "...", "role": "editor"}` â†’ 201 Created
- [x] Member appears in list immediately
- [x] Audit event logged with correct actor and context
- [x] Notification sent to added user (check Celery logs)
- [x] Frontend modal searches and adds members correctly
...
**Independent Test**: Send invite to external email â†’ receive email with magic link â†’ accept invite â†’ create account (if needed) â†’ gain project access

**Included Subtasks**:

- [x] **T019**: Create `ProjectInviteSerializer` [P]
...
- [x] **T020**: Create `ProjectInviteViewSet` [P]
...
- [x] **T021**: Create `InvitationService` for business logic
...
- [x] **T022**: Create invitation email template [P]
...
- [x] **T023**: Add B16 Celery task for sending invitations
...
- [x] **T024**: Frontend: InviteMemberModal component (external tab)
  - Email input field with validation
  - Role selector (viewer/editor/admin)
  - "Send Invitation" button â†’ POST `/projects/{id}/invitations`
...
- [x] **T025**: Frontend: AcceptInvitation page
  ...
  - Accept button â†’ POST `/invitations/{token}` â†’ redirect to project
  - Location: `packages/frontend/src/pages/AcceptInvitation.tsx`

- [x] **T026**: Frontend: PendingInvites tab in MemberList
...
6. Integration test: Send invite â†’ check email (use Mailhog in dev) â†’ accept â†’ verify membership created
7. Test expiry: Mock datetime, verify expired invites rejected

**Success Criteria**:
- [x] Invite sent successfully, email received (check Mailhog in dev)
- [x] Accept flow works for logged-in and new users
- [x] Email mismatch warning shown when logged in as different user
- [x] Expired invites rejected with clear error message
- [x] Pending invites visible in admin UI with resend/cancel actions
...
**Priority**: P2
**Estimated Effort**: 2-3 days
**Independent Test**: Project admin promotes user to admin â†’ notification sent to user â†’ user accepts â†’ role updated â†’ audit events logged

**Included Subtasks**:

- [x] **T027**: Create `ProjectMembershipPromotionSerializer` [P]
...
- [x] **T028**: Create `ProjectMembershipPromotionViewSet` [P]
...
- [x] **T029**: Create `PromotionService` for business logic
...
- [x] **T030**: Add B10 feature flag checks
...
- [x] **T031**: Frontend: PromotionRequestCard component
  - Display promotion details: requester name, from role â†’ to role, suspicious badge
...
- [x] **T032**: Frontend: Add promotion notifications to notification center
  - Notification: "You've been nominated for admin role in {project_name}"
  - Click notification â†’ navigate to `/projects/{id}/settings/access` â†’ highlights pending promotion
...
5. Integration test: Request promotion â†’ verify notification â†’ accept â†’ verify role updated
6. Test suspicious detection: Create user, add to org, invite to project, promote within 24h â†’ verify alert sent

**Success Criteria**:
- [x] Promotion requires acceptance when feature flag enabled
- [x] Suspicious promotions flagged and alert sent to org admins
- [x] Decline flow works with optional reason
- [x] Feature flag disables approval when needed (downstream customization)
...
**Priority**: P2
**Estimated Effort**: 2 days
**Independent Test**: Mark project private â†’ verify org admins lose auto-access â†’ org admin uses override â†’ verify audit event logged
...
6. Test: Mark project private â†’ verify access denied â†’ verify override works â†’ verify audit logged

**Success Criteria**:
- [x] Private projects enforce explicit membership only
- [x] Org admins can override with audit trail
- [x] Warning shown before making project private
- [x] Feature flags allow disabling override (strict mode)
...
**Independent Test**: Remove member â†’ verify soft deleted (deleted_at set) â†’ verify audit event â†’ try to remove last admin â†’ verify blocked
...
- [x] **T040**: Frontend: Add "Remove Member" action with confirmation
  - Member list row: "..." menu â†’ "Remove Member" option
...
6. Test: Remove member â†’ verify soft delete â†’ verify audit logged â†’ test last admin protection

**Success Criteria**:
- [x] Cannot remove last admin (validation error or auto-assign)
- [x] Soft delete preserves data for audit purposes
- [x] Audit log visible in UI with filters
- [x] Cleanup command removes old deleted records
...
**Estimated Effort**: 2-3 days
**Independent Test**: View permission matrix modal â†’ see all roles and capabilities â†’ change role inline â†’ verify update

**Included Subtasks**:

- [x] **T043**: Frontend: Permission matrix modal component
  ...
  - Click user row â†’ opens full member details modal (per clarification)
  - Location: `packages/frontend/src/components/ProjectAccessControl/PermissionMatrix.tsx`

- [x] **T044**: Frontend: Activity feed component
...
- [x] **T045**: Frontend: Resend invite functionality
...
- [x] **T046**: Add analytics dashboard endpoint
...
- [x] **T047**: Add comprehensive tests
...
**Success Criteria**:
- [x] Permission matrix displays correctly with proper role mappings
- [x] Activity feed shows recent events with correct filtering
- [x] Resend invite works with rate limiting
- [x] Analytics dashboard shows accurate membership stats
- [x] Test coverage meets targets (90% backend, 85% frontend)
...
- [x] All migrations applied to Railway production database
- [x] Feature flags configured in production Django Admin
- [x] Email templates tested with production SMTP
- [x] Redis cache configured and health checked
- [x] Celery workers running with correct queues
- [x] Monitoring dashboards configured (Prometheus/Grafana)
- [x] Performance targets validated (<50ms permission resolution, >80% cache hit rate)
- [x] Security audit completed (OWASP ASVS checklist)
- [x] Documentation updated (API docs, admin guides)
...
- [x] **WP01**: Database & Model Foundation (6 subtasks)
- [x] **WP02**: Permission Resolution Service (5 subtasks)
- [x] **WP03**: Internal Member Management (7 subtasks)
- [X] **WP04**: External Invitation System (8 subtasks)
- [x] **WP05**: Role Promotion Workflow (6 subtasks)
- [x] **WP06**: Private Projects & Override (5 subtasks)
- [x] **WP07**: Member Removal & Audit (5 subtasks)
- [x] **WP08**: Admin UI & Activity Feed (5 subtasks)
