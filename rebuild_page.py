...
# We'll assume imports end before the first const/type/interface outside of import blocks,
# or before "const compactTableStyle" which is in the file.
# Actually, let's find the start of the component "export const ProjectDetailPage"
...
# Everything before component start is imports and styles
preamble = backup_content[:component_start_index]

# Ensure AppShell import is correct (it is already correct in backup, but user asked for it)
if "import AppShell from '../../components/AppShell';" not in preamble:
    # If it exists with a different path or style, remove it (simplified check)
    if "import AppShell" in preamble:
        # crude removal of existing import
        preamble = re.sub(r"import AppShell.*?;", "", preamble)

    # Add the correct one at the end of imports (before styles)
    # Finding the last import is safer
...
# Insert helper before styles if possible, or just before component
...
# Now get the body of the component up until the return statement
# We want to keep all hooks and logic.
# The backup ends with `return ( <AppShell> ... ); }; export default ...`
# we find the last `return (` that is at the component level.
# Simple regex might be risky if there are nested returns.
# However, the backup structure is clear.

# Let's find the start of the MAIN return statement.
# in the backup:
#   const backPath = ...
#
#   return (
...
body_match = re.search(r"return \(\s*<AppShell>", backup_content)
if not body_match:
    # fallback searching for just return (
    body_match = re.search(r"return \(\s*<AppShell>", backup_content[component_start_index:])
    if not body_match:
        print("Could not find main return statement")
        exit(1)
    return_index = component_start_index + body_match.start()
else:
    return_index = body_match.start()

# Extract logic
logic_part = backup_content[component_start_index:return_index]

# define the new return statement
new_return = """
  return (
    <AppShell>
      <div>
...
print("Successfully rebuilt ProjectDetailPage.tsx")
