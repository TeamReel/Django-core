...
class PIIRedactionFilter(logging.Filter):
    ...
    def _redact_dict(self, data: dict) -> dict:
        ...
        for key, value in data.items():
            ...
            if key.lower() in self.REDACTED_FIELDS or any(
                pattern in key.lower() for pattern in ["_token", "_secret", "_key", "_password"]
            ):
    ...
    def _redact_string(self, text: str) -> str:
        ...
        text = re.sub(r"\b\d{3}-\d{2}-\d{4}\b|\b\d{9}\b", "[REDACTED_SSN]", text)
        return text
