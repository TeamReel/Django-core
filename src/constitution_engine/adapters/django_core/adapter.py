...
class DjangoAdapter:
    ...
    def _discover_apps(self) -> list[dict[str, str | Path]]:
    ...
    def get_app_paths(self) -> list[Path]:
        ...
        """
        apps = self._discover_apps()
        return [Path(app["path"]) for app in apps]
    ...
    def get_project_structure(self) -> dict[str, list[Path]]:
        ...
        return {
            ...
            "manage_py": [self.config.manage_py_full_path] if self.config.manage_py_full_path.exists() else [],
        }
    
    @classmethod
    def from_project_root(cls, project_root: Path | str, **config_overrides) -> DjangoAdapter:
