...
def process_match_intro_video(self, job_id: str) -> str | None:
    """Process match intro video via Celery.
    ...
    try:
        ...
        logger.error("Match intro job not found", extra={"job_id": job_id})
        return None

    if job.status in (JobStatus.COMPLETED, JobStatus.CANCELLED):
        logger.info(
            "Match intro job already processed or cancelled",
    ...
    try:
        ...
        logger.exception(
            "Match intro video processing failed",
