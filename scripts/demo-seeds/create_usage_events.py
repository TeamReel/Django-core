...
def create_sample_events():
    ...
    if not users:
        print("❌ No users found. Run setup_football_data.py first.")
        return

    if not orgs:
        print("❌ No organisations found. Run setup_football_data.py first.")
        return

    print(f"📊 Found {len(users)} users, {len(orgs)} organisations, {len(projects)} projects")

    # Football-specific event types
    event_types = [
        (
            "match.scheduled",
        ...
        (
            "injury.report",
            {"player": "Player Name", "type": "muscle", "expected_recovery_days": 14},
        ),
    ...
    for day in range(30):
        ...
        num_events = random.randint(8, 20)  # More events per day
        ...
        for _ in range(num_events):
            ...
            # Add some variation to timestamps within the day
            event_timestamp = (
                datetime.now()
                - date_offset
                - timedelta(
    ...
    print(f"✅ Created {created} football-related usage events")
...
if __name__ == "__main__":
    ...
    print("\n✨ Done! Refresh the Usage Events page to see the data.")
