...
if ($typeCheckSuccess) {
    Write-Host "[PASS] Type-check passed ($([Math]::Round($typeCheckTime, 2))s)" -ForegroundColor Green
}
else {
...
if ($lintSuccess) {
    Write-Host "[PASS] Lint passed ($([Math]::Round($lintTime, 2))s)" -ForegroundColor Green
}
else {
