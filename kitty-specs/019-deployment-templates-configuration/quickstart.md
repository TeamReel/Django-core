...
```nginx
# See contracts/nginx-production.conf for full example
server {
    listen 80;
...
See [.env.example](.env.example) for complete list with validation rules.
...
docker-compose -f docker-compose.local.yml up -e WEB_PORT=8001
```
...
```bash
# Ensure source code volume mounted
...
# Verify Gunicorn listening on 8000
...
| Issue | Solution |
|-------|----------|
| Permission denied | Run as non-root user (UID 1000) |
| Image pull errors | Verify image registry credentials |
| Migration lock errors | Only one migration process at a time |
| Celery not processing | Check CELERY_BROKER_URL connectivity |
| High memory usage | Tune Gunicorn workers, Celery concurrency |
