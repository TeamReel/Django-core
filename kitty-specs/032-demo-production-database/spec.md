# Feature Specification: Demo Production Database & Seed Data

**Feature Branch**: `032-demo-production-database`
**Created**: 2025-12-17
**Status**: Draft
**Module**: #032 F10b-Database (Fase 8: Demo Foundation)

## Clarifications

### Session 2025-12-17

- Q: Project count precision - Requirements state "70+ projects" but E2E tests need exact counts for assertions. Should the seed data generate exactly 70 projects, or allow variation? → A: Exactly 80 projects (matches math: 15+30+10+5+20)
- Q: Audit event count precision - Requirements state "200+ audit events" but E2E tests need predictable expectations. Should the seed data generate exactly 200 events, or allow a range? → A: Range 200-300 (semi-random per run)
- Q: Transaction history window - Seed data mentions transactions over differing windows. Which window should transactions use? → A: 30 days
- Q: Notification distribution - Should unread notifications be fixed per demo account or a range? → A: Range 5-10 unread per account (seeded)

- Q: Which organizations are premium vs trial for feature flags? → A: DataLab and AI Research Inc are premium; TechCorp, MarketingHub, and OpenSource Collective are trial.
- Q: How do we ensure realistic names and avoid lorem ipsum? → A: Use curated lists for organization, project, and user display names; absolutely no lorem ipsum.
...
A developer clones the repository and wants to see a fully functional demo with realistic data in under 60 seconds without manually creating test accounts or organizations.

**Why this priority**: This is the primary value proposition - instant demo readiness. Without this, developers would spend hours creating test data manually, blocking all downstream work.

**Independent Test**: Clone repo → run `docker-compose --profile demo up` → wait <60s → visit http://localhost:8080 → log in with demo account → see 5 organizations, 20 users, 80 projects with realistic activity.

**Acceptance Scenarios**:

1. **Given** a fresh repository clone, **When** developer runs `docker-compose --profile demo up`, **Then** PostgreSQL + Django + seed data are ready in <60 seconds
