...
De keeper-positie (GK, slot 1) kan gevuld worden met elke speler, ook als die geen keeper-tenue assets heeft. Omgekeerd: een member met alleen keeper-tenue kan op een veldpositie gezet worden die thuis-tenue vereist. De video toont dan placeholder silhouetten.
...
- Na H0: `Formation.positions` definieert welke slot `GK` is (waar `line="keeper"`)
...
- [ ] Voeg validatie toe in `LineupSyncService.sync()`:
  - Lees `Formation.positions` → bepaal welke slots `line="keeper"` hebben
  - Voor keeper-slots: check of member processed `goalkeeper` fullbody asset heeft
  - Voor veldposities: check of member processed `home` fullbody asset heeft
...
- [ ] Update squad groepering in `useLineupSheet.ts`:
  ...
  - Members met alleen keeper-tenue verschijnen in keeper-groep
...
- [ ] Test: video builder gebruikt correcte kit_type per slot (ongewijzigd)

## Done criteria

- [ ] Keeper-slot Participations bevatten `data.asset_warning` als keeper-tenue ontbreekt
