...
class TestSearchAPI:
    ...
    def test_global_search_grouping(self, mock_backend_cls):
        ...
        # We can mock the return value of annotate to be the list
        mock_qs = MagicMock()
        mock_backend.search.return_value = mock_qs
        mock_qs.annotate.return_value = [entry1, entry2]
...
class TestSearchAPIIntegration:
    ...
    def test_filtered_search_with_pagination(self, client, test_user, test_project):
        ...
        # Create multiple project search entries
        ...
        for i in range(25):
            SearchEntry.objects.create(
                ...
                url=f"/projects/{i}/",
                body_text=f"Project {i} test data",
            )

        client.force_login(test_user)

        # Request with types filter
        response = client.get(self.url, {"q": "project", "types": "projects.project"})
    ...
    def test_highlighting_in_results(self, client, test_user, search_entries):
        ...
        # Find any result with highlight
        has_highlight = False
        for category, results in data.items():
            for result in results:
                if "highlight" in result and result["highlight"]:
                    # Check for HTML highlight tags
                    assert "<b>" in result["highlight"] or result["highlight"] is None
                    if "<b>" in str(result["highlight"]):
                        has_highlight = True
    ...
    def test_grouped_results_respect_limit(self, client, test_user):
        ...
        response = client.get(self.url, {"q": "searchuser"})
